...
class RestFormatTest extends TestCase {
  ...
  public function json_serialize() {
    ...
    RestFormat::$JSON->write($res, new \webservices\rest\Payload(array('name' => 'Timm')));
  ...
  public function json_deserialize() {
    ...
    $v= RestFormat::$JSON->read($req, \lang\MapType::forName('[:string]'));
  ...
  public function xml_serialize() {
    $res= new MemoryOutputStream();
    RestFormat::$XML->write($res, new \webservices\rest\Payload(array('name' => 'Timm')));
    $this->assertEquals(
